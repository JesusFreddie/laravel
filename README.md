...
После подключения к удаленному серверу, выполните эту команду чтобы подключиться к контейнеру базы данных и введите пароль (при вводе пароля, пароль он не должен отображаться)

```sh
docker exec -it <id контейнера> mysql -h 127.0.0.1 -u root -p
```

После вы попадете в консоль sql, где можете исполнять нужные команды, например:

```sh
SHOW DATABASES;
USE laravel_db;
SELECT * FROM users;
INSERT INTO users (email, name, role) VALUES ('anna_mamonta@mail.ru', 'Anna Mamontova', 'admin');
```

Чтобы выйти из контейнера команда:
